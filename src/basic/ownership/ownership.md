...
总之 `String` 类型指向了一个堆上的空间，这里存储着它的真实数据, 下面对上面代码中的 `let s2 = s1` 的拷贝行为分成两种情况尽心假设：
...
这段代码和之前的 `String` 有一个本质上的区别：在 `String` 的例子中 `s1` 持有了通过`String::from("hello")` 创建的值的所有权，而这个例子中，`x` 通过 `&` 符号创建了一个引用，`x` 只是引用了存储在二进制中的字符串 `"hello, world"`，并没有持有所有权。
